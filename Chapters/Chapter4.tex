...
TEP problems usually considers to build new lines in existing connections so that we not changing the network topology. One of the reasons to add lines in existing connections is because the legal permission are granted. However, in a future scenario with a high share of decentralized renewable energies, new connections have to be considered. Hence, we are going to able new connections by changing the topology. Moreover, we allow all lines to be connected to each node, i.e., we are considering fully connected networks.
...
\begin{itemize}
    \item \textbf{Objective function Eq.\,\ref{eq: objective}}: it is the sum of investment cost of transmission lines, operational cost of generators and load shedding cost.
...
Notice, that when the solution is feasible, i.e., $r_{k} = 0\ quad \forall k\in N$,then the total cost depends on the investment cost and the operational cost. As stated before, the number of possible configurations of our problem scales badly. For this reason, knowing if we are considering a large set of snapshots -- large investment planning - or a small subset of snapshots gives us clues about what region of the configuration space we should explore. There are two regions to be considered:
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Small Network on D-Wave's annealer}
 We are considering a single snapshot of a three node fully connected network with non-existing lines with a single demand at node $2$ and without the load shedding term. Each node has a single generator $g_{k}$ of with a nominal power -- maximum capacity -- of $10$ energy units.
 \begin{table}[H]
\centering
\begin{tabular}{ |c ||c| c| }
  \hline			
...
\begin{mini!}[2]
	{\mathbf{x}, \mathbf{g}}{\underbrace{\mathbf{C}^{\intercal}\mathbf{x}}_{\text{Investment Cost}} + \underbrace{\mathbf{C}_{\text{oc}}^{\intercal}\mathbf{g}}_{\text{Operational Cost}}}{}{}{}
 	\addConstraint{g_{k}}{\leq \bar{g}_{k} }{}{\forall\, k \in N}
	\addConstraint{d_{k}}{= g_{k}  + \underbrace{\sum_{l\neq k, l\in N}g_{k}x_{kl}}}_{\text{Quadratic Constraint}}{}{\forall\, k \in N}
    \addConstraint{\mathbf{d}, \mathbf{g}}{\geq 0\,}{}{}
    \addConstraint{x_{kl}}{\in \{0,1\},\,}{\forall\, kl \in C}.
    \end{mini!}
Notice that we are not considering the power flow limits $f_{kl}^{1}$ of candidate lines. Any candidate line can transmit any amount of energy. In other words, we wrote the problem in such a way that none power flow limits is going to be violated, see Figure\,\ref{fig: Green_inital}.
\begin{figure}[H]
  \begin{center}
...
\end{figure}
The demand at node $2$ is $12$ energy units, so it cannot be fulfilled by its own generator and require energy from at least one of the neighboring nodes which means we need to build at least one transmission line connected to node $2$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
