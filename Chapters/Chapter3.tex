...
\begin{mini!}[2]
	{\mathbf{x}}{f(\mathbf{x})}{\label{eq: MILP}}{}{\left[f: \mathbb{Z}^{n} \rightarrow \mathbb{R}\right]}
	\addConstraint{h(\mathbf{x})}{=0,}{\left[h: \mathbb{Z}^{n} \rightarrow \mathbb{R}^{m}\right]}
	\addConstraint{g(\mathbf{x})}{\leq 0,\quad}{\left[g: \mathbb{Z}^{n} \rightarrow \mathbb{R}^{p}\right],}
\end{mini!}
...
\begin{maxi!}[2]
	...
	\addConstraint{(\lambda, \mu)}{}{\in \{\lambda, \mu \,\mid\, q(\lambda, \mu)\,  \text{Bounded} \}.}
\end{maxi!}
...
\end{proof}
%%%%%%%%%%%%%%
\subsubsection{Duality points of view: energy market example.}
Suppose that an electricity market has to sells its energy production to different customer. However, the maximum amount of energy per customer is fixed to $10$ units of energy where each unit of energy cost $1$\$.\\
The electricity company decides to relaxed the constraints allowing the customer Bob to buy more than $10$ units of energy but he has to pay $2$\$ for each extra unit of energy. Bob as customer decide to buy extra units of energy because with one unit of energy he produces a chip that he sells for $3$\$, so that he decides to buy every extra energy unit he can by getting a profit of $1$\$ for each unit of energy he buys. The electricity market notice that Bob is buying too much energy and it cannot provide the require energy to other customers, so that the electricity market pays a fine because of that. In order to relaxed the constraints but at the same time satisfy the demand of other customers, the electricity market decides to increase the cost of the extra unit of energy to $3$\$. Now, the current prices of extra energy units makes Bob generates zero profit so he is no longer interested in buying extra units of energy. In other words, Bob has no interest in violating the constraint, despite this constraint is relaxed and he could violated if he wish.\\\\
Notice that we have two points of view when solving combinatorial optimization problem. We can be the customer, who is the one solving the problem, and of the one who defines the rules, i.e., the one who sets the constraints and maximize the prices.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Benders' Cuts}
Now we are going to introduce Benders' cuts which are the key component of Benders' decomposition algorithm. Suppose we are given the following MILP problem,
\begin{mini!}[2]
	{x,y}{z = -y - x}{}{}{\label{eq: Exampleobjective}}
    \addConstraint{a\cdot x}{= y \,}{}{\label{const: LinkingConstrain}}
    \addConstraint{b \cdot x}{\leq c}{}
\end{mini!}
with $a,b,c \in \mathbb{R}$. Equation\,\ref{const: LinkingConstrain} represents a linking constraint since it mix both decision variables. Considering $x$ as the complicated variable we can fix it into a feasible value so that we simplified the problem,
\begin{mini!}[2]
	{y}{- y}{}{}{}
    \addConstraint{a\cdot x_{\text{fixed}}}{= y \,}{}{}
    \addConstraint{b \cdot x_{\text{fixed}}}{\leq c}{}
\end{mini!}
Notice that the last constraint is already satisfied for a feasible value of $x$, so
\begin{mini!}[2]
	{y^{t}}{- y^{t}}{}{}{}
    \addConstraint{a\cdot x^{t}_{\text{fixed}}}{= y^{t} \,}{}{}
\end{mini!}
where $t$ indicates Benders iteration.
%%%%%%%%%%%%%%%%%%%
