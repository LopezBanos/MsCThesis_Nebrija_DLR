...
A classical bit has two possibles states commonly named $\ket{0}$ or $\ket{1}$. Notice that the name of the states of a bit or qubit is not important. It is just a way of labelling those states. However, a quantum bit is a linear combination of states -- \textit{superposition} -- $\{\ket{0}, \ket{1}\}$. The general state of a qubit can be conceived as a vector in a two-dimensional complex vector space, with $\alpha, \beta \in \mathbb{C}$,
\begin{equation}
...
\end{equation}
where $\hat{\mathcal{H}}$ is the Hamiltonian operator, $\hbar$ is the reduced Planck constant and $\ket{\psi(t)}$ represents the state of a system and $\ket{\psi(t)}$ represent the state of a system.\\
